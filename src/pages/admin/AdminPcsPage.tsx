...
function exportCsv(pcs: Teacher[], usernameLookup: Map<string, string>) {
  ...
  a.href = url;
  a.download = "performance-coaches.csv";
  a.click();
  URL.revokeObjectURL(url);
}

// The "PC" page — the same underlying teachers list as /admin/teachers, but
// showing only performance coaches (is_performance_coach = true). A row opens
// the PC detail view at /admin/pcs/:id (Details / Students / Report tabs), not
// the plain teacher record.
export default function AdminPcsPage() {
  ...
  const uniqueCountries = [...new Set([...activePcs, ...inactivePcs].map((t) => t.country).filter(Boolean) as string[])].sort();
  ...
  const filtered = applyFilters(activePcs);
  const filteredInactive = applyFilters(inactivePcs);
  ...
  return (
    <AdminLayout>
      <PageHeader
        title="PC"
        description="Performance coaches only — the teachers flagged as a Performance Coach."
        action={
          <div className="flex gap-2">
            ...
            <Button onClick={() => navigate("/admin/teachers/new?pc=1")} className="flex items-center gap-2">
              <IconPlus /> Add PC
            </Button>
          </div>
        }
      ...
      {tab === "active" && (
        <DataTable
          ...
          getRowId={(t) => t.id}
          loading={loading}
          onRowClick={(t) => navigate(`/admin/pcs/${t.id}`)}
        />
      )}
