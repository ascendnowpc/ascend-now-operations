...
// Sends an email to the session coordinator (PC) when a no-show is logged.
//
// Required edge function secrets:
//   SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, SMTP_FROM
//
// Deploy: supabase functions deploy notify-no-show
...
serve(async (req) => {
  ...
  try {
    ...
    if (!log.no_show_type) return json({ error: "Not a no-show session" }, 400);
    ...
    const noShowLabel = NO_SHOW_LABELS[log.no_show_type] ?? log.no_show_type;
    ...
    await client.send({
      ...
      subject: `No Show Alert - ${studentDisplay} (${noShowLabel})`,
      html: `<div style="font-family:Arial,sans-serif;max-width:540px;color:#1e293b;">
<div style="background:#1e3a5f;padding:20px 24px;border-radius:8px 8px 0 0;">
<p style="margin:0;font-size:11px;color:#93c5fd;text-transform:uppercase;letter-spacing:1px;">Ascend Now - Session Alert</p>
<h2 style="margin:6px 0 0;font-size:20px;color:#ffffff;">No Show Alert</h2>
</div>
<div style="border:1px solid #e2e8f0;border-top:none;border-radius:0 0 8px 8px;padding:24px;">
<p style="margin:0 0 20px;font-size:14px;color:#475569;">Hi <strong>${coordinatorName}</strong>, a no-show session has been logged and requires your attention.</p>
<table style="width:100%;border-collapse:collapse;font-size:14px;">
<tr style="background:#f8fafc;">
<td style="padding:10px 14px;color:#64748b;font-weight:600;width:120px;border:1px solid #e2e8f0;">Type</td>
<td style="padding:10px 14px;border:1px solid #e2e8f0;"><span style="background:#fef3c7;color:#92400e;padding:2px 10px;border-radius:999px;font-size:12px;font-weight:700;">${noShowLabel}</span></td>
</tr>
<tr>
