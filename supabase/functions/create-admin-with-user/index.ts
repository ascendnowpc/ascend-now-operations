// Supabase Edge Function: create-admin-with-user
// Creates a Supabase Auth account + public.users row (via the
// trg_create_user_profile trigger, which reads role from user_metadata) +
// an admins marker row in one atomic-ish operation.
// Only callable by authenticated admins.
//
// Deploy: supabase functions deploy create-admin-with-user
...
serve(async (req) => {
  ...
  try {
    ...
    //    automatically and creates the public.users row with role = 'admin'
    //    (read from user_metadata.role).
    ...
    // 3. Email the new admin their login credentials — sent in the background
    // (not on the request/response critical path) so the creating admin isn't
    // stuck waiting on an SMTP connection; still non-fatal if it fails.
    runInBackground(
      sendWelcomeEmail({ first_name, last_name, email, username, password })
...
async function sendWelcomeEmail(opts: {
  ...
  await client.send({
    ...
    to: opts.email,
    subject: "Welcome to Ascend Now - your admin login details",
...
<p style="margin:0 0 20px;font-size:14px;color:#475569;">Hi <strong>${fullName}</strong>, your Ascend Now admin account has been created. Here are your login details:</p>
