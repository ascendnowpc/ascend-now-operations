// Supabase Edge Function: notify-homework-grade-published
// Fired (fire-and-forget) right after a teacher publishes a grade to the
// student (useSubmissionGrade.publishGrade() -> grades.published_at set).
// Emails the student that their marks/feedback are ready to view.
//
// Recipient: students.notification_email, falling back to students.email —
// same fallback used by notify-homework-published/notify-homework-submitted.
//
...
//
// Deploy: supabase functions deploy notify-homework-grade-published
...
serve(async (req) => {
  ...
  try {
    ...
    const { grade_id } = await req.json();
    if (!grade_id) return json({ error: "grade_id required" }, 400);

    const { data: grade, error: gradeError } = await serviceClient
      .from("grades")
      .select("id, submission_id, total_marks, max_marks, published_at")
      .eq("id", grade_id)
      .single();

    if (gradeError || !grade) return json({ error: "Grade not found" }, 404);
    if (!grade.published_at) return json({ error: "Grade is not published" }, 400);

    const { data: submission, error: subError } = await serviceClient
      .from("submissions")
      .select("id, paper_id, student_id")
    ...
    const hasMarks = grade.total_marks != null && grade.max_marks != null;
    ...
    try {
      await client.send({
        ...
        to: recipient,
        subject: `Your Homework Has Been Graded${subjectLabel ? ` — ${subjectLabel}` : ""}`,
...
<h2 style="margin:6px 0 0;font-size:20px;color:#ffffff;">Your Homework Has Been Graded</h2>
</div>
<div style="border:1px solid #e2e8f0;border-top:none;border-radius:0 0 8px 8px;padding:24px;">
<p style="margin:0 0 20px;font-size:14px;color:#475569;">Hi <strong>${studentName}</strong>, your teacher has reviewed and published your grade.</p>
...
${hasMarks ? `<tr>
<td style="padding:10px 14px;color:#64748b;font-weight:600;border:1px solid #e2e8f0;">Score</td>
<td style="padding:10px 14px;border:1px solid #e2e8f0;"><span style="background:#dcfce7;color:#166534;padding:2px 10px;border-radius:999px;font-size:12px;font-weight:700;">${grade.total_marks} / ${grade.max_marks}</span></td>
</tr>` : ""}
</table>
${homeworkUrl ? `<div style="text-align:center;margin:28px 0 8px;">
<a href="${homeworkUrl}" style="display:inline-block;background:#0284c7;color:#ffffff;text-decoration:none;font-weight:600;font-size:14px;padding:12px 28px;border-radius:8px;">View Your Grade</a>
</div>
