-- Data migration: re-seed the student Batu Ozcelik, his six subject teachers,
-- their IBMYP subject links, and his 100-hour Academic package.
--
-- This replays 20260728120000_seed_batu_ozcelik_student_and_teachers.sql, whose
-- rows were all removed by the 2026-08-03 purge
-- (20260803000000_purge_teachers_students_sessions.sql). The original migration
-- file is left untouched — past migrations are never edited — so the data is
-- restored through this new one instead.
--
-- Prerequisites, both satisfied by earlier migrations in this batch:
--   * 20260804000000 restored trg_create_user_profile on auth.users, without
--     which the auth.users inserts below would not produce public.users rows
--     and every teachers/students insert would fail on its user_id FK.
--   * 20260804000100 created the performance coaches, including Rana Walid,
--     whom the pc_student_assignments insert below looks up by email.
--
-- Source: the client's "Batu" session-log sheet. The sheet only carries each
-- teacher's first name, so the teachers are created with that exact first name
-- and placeholder contact details. One of them (Anugya) uses the client's own
-- address so she can actually be logged into.
--
-- Idempotent: each teacher is skipped if their email already exists, and the
-- student block is skipped if Batu is already present.
